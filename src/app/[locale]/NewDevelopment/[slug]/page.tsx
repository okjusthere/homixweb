...
export default async function NewDevelopmentDetailPage({
  ...
  const jsonLd = {
    ...
    name: building.name,
    url: absUrl(localizePath(locale, `${newDevelopmentBasePath}/${building.slug}`)),
    address: {
      "@type": "PostalAddress",
    ...
    description: content?.overview[locale] ?? developmentIntro(building, locale),
    ...
    <Container className="py-8 sm:py-12">
      ...
      <header className="mt-8 border-b border-line pb-8">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {building.flexiblePayment && (
            <Link
              href="/flexible-payment-homes"
